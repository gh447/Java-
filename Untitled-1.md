...
！[MethodOfJava](./assets/MethodOfJava.jpg)
...
public double cost(int numToBuy){
    int fullPriceCount = numToBuy/2+numToBut%2;
    int halfPriceCount = numToBUy - fullPriceCount;
    double totalCost = fullPriceCount * soldPrice + 
    halfPriceCount * soldPrice/2
    return totalCost;
}
...
double是返回值类型，无需返回值用void，void是Java中的关键字。

cost是方法名，任意的合法标识符都可以。

numToBuy是参数，int是参数类型，参数可以使任何类型，甚至是自己的类型。

有返回值的方法会return语句，return语句必须是代码块的最后一句。

这个方法实现了在超市第二件半价情况下的总花费
